//! â€” Migration dry-run on disposable PG via testcontainers.
//!
...
pub async fn run_dryrun_against(    host: String,
    ...
    if tracking_enabled {
        ...
        for row in &seed {
            ...
            // `now()` (`Utc::now()`) if the source string is malformed â€”
    ...
    // Step 7: aggregate Squawk findings (per design Â§5.1.7 â€” visibility into
    ...
    DryRunReport {
        success,
        container_pull_ms: 0,  // not measured here â€” caller fills in
        container_start_ms: 0, // not measured here â€” caller fills in
        steps,
        total_ms: i64::try_from(total_start.elapsed().as_millis()).unwrap_or(i64::MAX),
        findings,
        error,
...
pub async fn migrations_dryrun(    app: AppHandle,
    ...
    let mut report = tokio::select! {
        ...
        _ = &mut cancel_rx => {
            ...
            unregister(&state, &conn_id).await;
            // Container handle goes out of scope here â†’ testcontainers Drop
...
mod tests {
    ...
    async fn dryrun_seeds_ledger_so_already_applied_dont_rerun() {
        ...
        // Pretend 0001 is already applied on prod â€” pass it as a seed row.
