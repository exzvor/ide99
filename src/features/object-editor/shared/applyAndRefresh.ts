...
export async function applyAndRefresh(tabId: string, connId: string, ddl: string): Promise<void> {
  ...
  // 2.5) S28 — invalidate hypertable registry. The apply may have created
  ...
  // 2.6) S29 — invalidate pg_partman registry. The apply may have created
  ...
  // 3) Schema tree refresh — fire-and-forget so the apply call resolves
