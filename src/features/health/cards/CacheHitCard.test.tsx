...
describe("CacheHitCard", () => {
  ...
  it("ready: warn for 85 ≤ ratio < 95", () => {
    ...
    expect(screen.getByTestId("health-card-cache_hit-status").getAttribute("data-tone")).toBe(      "warn",
);
  });
  ...
  it("ready: danger for ratio < 85; horizontal bar present", () => {
    ...
    expect(screen.getByTestId("health-card-cache_hit-status").getAttribute("data-tone")).toBe(      "danger",
);
