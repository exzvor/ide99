...
describe("buildExplainSql", () => {
  it("plain EXPLAIN with minimum options", () => {
    expect(buildExplainSql("SELECT 1", baseOpts())).toBe(
      "EXPLAIN (FORMAT JSON, BUFFERS, SETTINGS) SELECT 1",
    );
  });
  ...
  it("drops WAL + TIMING when mode != analyze", () => {
    expect(buildExplainSql("SELECT 1", baseOpts({ wal: true, timing: true }))).toBe(
      "EXPLAIN (FORMAT JSON, BUFFERS, SETTINGS) SELECT 1",
    );
  });
